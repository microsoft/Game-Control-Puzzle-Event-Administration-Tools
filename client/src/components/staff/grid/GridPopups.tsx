...
export const GridUnlockedPopup = ({ solveData, clues, onRelock, onComplete }: UnlockedProps) => {
    ...
    return (
        <>
            <p>
                This {clueType} was unlocked at <em>{moment.utc(solveData.startTime).local().format('HH:mm')}</em>. Our wild guess is that it will be solved around{' '}
                <em>{moment.utc(solveData.predictedEnd).local().format('HH:mm')}</em>.
            </p>
            <p>
            ...
            <p>
                {!!solveData.unlockedClues && solveData.unlockedClues.length > 0
                    ? 'Solving this will currently unlock the following: ' +
                      solveData.unlockedClues.map((unlock) => ' ' + clues.find((x) => x.submittableId === unlock).submittableTitle + ' ')
    ...
    return (
        <>
            {solveData.predictedStart ? (
                <p>
                    According to our random number generated by Game Control, we believe this team will unlock this puzzle at{' '}
                    <em>{moment.utc(solveData.predictedStart).local().format('HH:mm')}</em> and solve it at <em>{moment.utc(solveData.predictedEnd).local().format('HH:mm')}</em>
                </p>
            ) : (
                <p>According to our predictive algorithm this team has no path to unlocking this puzzle.</p>
            )}
            <p>
                {solveData.unlockedClues?.length > 0
                    ? 'Solving this will currently unlock the following: ' +
                      solveData.unlockedClues.map((unlock) => ' ' + clues.find((x) => x.submittableId === unlock).title + ' ')
                    : 'Solving this will not unlock anything.'}
            </p>
            <p>
                <Button
