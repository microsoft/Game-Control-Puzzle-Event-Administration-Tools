...
export const GridCell = ( { solveData, teamName, clues, puzzleTitle, onRelock, onUnlock, onSkip }: Props) => {
    if (solveData.isSkipped) {
        ...
        return (
            <td key={solveData.clueId + solveData.teamId} style={{backgroundColor: "orange"}} className="staffGridCell staffGridCellIcon staffGridCellSkipped">
                <DialogRenderProp
                    ...
                    renderButton={() => <div><span role="img" aria-label="skipped">&#x23E9;</span></div>}
        ...
        return (
            <td key={solveData.clueId + solveData.teamId}
                className="staffGridCell staffGridCellLocked">  
                <DialogRenderProp
                    variant="link"
                    renderTitle={() => `${teamName} has no path to unlock ${puzzleTitle}`}
                    renderButton={() => <div className="staffGridUnreachable"><MdLock/></div>}
                    renderBody={(onComplete: () => void) =>
                        <GridUnlockablePopup
                            solveData={solveData}
                            onUnlock={onUnlock}
        ...
        return (
            <td key={solveData.clueId + solveData.teamId}
                ...
                style={{backgroundColor: backgroundColor}} title="predicted">
                <DialogRenderProp
                    variant="link"
                    size="small"
        ...
        {
            if (clue.submittableType.trim() === "locunlock" &&
                moment.duration(moment.utc().diff(moment.utc(solveData.startTime))).asMinutes() > (clue.parSolveTime + 15))
            {        
                badge = <>&nbsp;<span className="badge badge-pill" title="Slow Team Alert">!</span></>            
            } else if (clue.submittableType.trim() === "Puzzle" &&
                moment.duration(moment.utc().diff(moment.utc(solveData.startTime))).asMinutes() > (clue.parSolveTime + 30))
            {
                badge = <>&nbsp;<span className="badge badge-pill" title="Slow Team Alert">!</span></>                                
            }
        }
        ...
        return (
            <td 
                className="staffGridCell staffGridCellUnlocked staffGridCellTime"
                ...
                <DialogRenderProp
                    ...
                    renderButton={() => <div>{moment.utc(moment.duration(moment.utc().diff(moment.utc(solveData.startTime))).as('milliseconds')).format('HH:mm:ss')}{badge}</div>}
                    renderBody={(onComplete: () => void) => 
                        <GridUnlockedPopup
                            solveData={solveData}
                            onRelock={onRelock}
