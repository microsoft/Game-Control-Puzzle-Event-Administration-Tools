...
export const ContentForm = ({ content, achievements, onSubmit }: Props) => {
  ...
  const [achievementUnlockId, setAchievementUnlockId] = useState(
    content?.achievementUnlockId ?? undefined
  );
    ...
    <>
      <FormGroup>
        ...
        <Form.Text className="text-muted">
          Use 'UnsolvedPlot' or 'SolvedPlot' for plot content.
        </Form.Text>
      ...
      {contentType === "RichText" && (
        <FormGroup>
          ...
          />
          <h6>
            To insert a unique id in the URL, use [[player]], [[team]] or
            [[shortname]] as a replacement token.
          </h6>
        </FormGroup>
      )}
      {contentType !== "Image" && contentType !== "RichText" && (
        <FormGroup>
          ...
          <Form.Control.Feedback type="invalid">
            Content must not be empty, and if a youtube URL must include embed
            in the URL
          </Form.Control.Feedback>
        </FormGroup>
      )}
      ...
      <FormGroup>
        ...
        >
          ...
          {achievements?.map((achievement) => (
            <option value={achievement.achievementId}>
              {achievement.name}
            </option>
          ))}
        </Form.Control>
