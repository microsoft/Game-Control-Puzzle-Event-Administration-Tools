    ...
    return (
        <>
            <FormGroup>
                ...
                <Form.Control type="text" required value={contentName} isInvalid={!isNameValid()} onChange={(event) => setContentName(event.target.value)} />
                <Form.Text className="text-muted">Use 'UnsolvedPlot' or 'SolvedPlot' for plot content.</Form.Text>
                <Form.Control.Feedback type="invalid">Name is required</Form.Control.Feedback>
            </FormGroup>
            <FormGroup>
                <Form.Label>Content Type</Form.Label>
                <Form.Control as="select" disabled={!!content} value={contentType} onChange={(event) => setContentType(event.target.value)}>
            ...
            {contentType === 'Image' && (
                <FormGroup>
                    ...
                    <Form.Control type="file" onChange={(event: any) => setBinaryContent(event.target.files[0])} />
                </FormGroup>
            )}
            ...
            {contentType === 'RichText' && (
                <FormGroup>
                    ...
                    <CodeMirror value={stringContent} onChange={(value) => setStringContent(value)} />
            ...
            {contentType !== 'Image' && contentType !== 'RichText' && (
                <FormGroup>
                    ...
                    <Form.Control type="text" value={stringContent} isInvalid={!isTextContentValid()} onChange={(event) => setStringContent(event.target.value)} />
                    <h6>To insert a unique id in the URL, use [[player]], [[team]] or [[shortname]] as a replacement token.</h6>
            ...
            <FormGroup>
                ...
                <Form.Control as="select" disabled={!!content || !achievements} value={achievementUnlockId} onChange={(event) => setAchievementUnlockId(event.target.value)}>
                    <option value="" selected>
                        None
                    </option>
                    {achievements?.map((achievement) => (
                        <option value={achievement.achievementId}>{achievement.name}</option>
                    ))}
                </Form.Control>
            </FormGroup>
