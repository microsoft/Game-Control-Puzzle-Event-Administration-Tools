...
const getFriendlyNameForCallType= (call) =>
{
    const ctype = call.callType ?? "";
    switch (ctype.toLowerCase().trim())
    {
        case "checkin": return "Routine Check-In";
        case "teamfree": return "Puzzle Request Button";
        case "teamhelp": return "Help Request Button";
        case "hint":  return "Hint";
        case "confirm":  return "Confirm Data";
        case "other": return "Other";
        default:
        case "none": return "Unknown or unspecified type";
    }
}

const CallList = ({currentTeam}) => {
    if (currentTeam && currentTeam.callHistory) {
                        ...
                        <div>
                            <Row>
                                <Col xs={8} md={8} style={{textAlign: "left"}}>
                                    ...
                                    {call.notes && <div><small>GC Internal Notes:  {call.notes}</small></div>}
                                    {call.teamNotes && <div><small>Team said to GC:  {call.teamNotes}</small></div>}
                                    {call.publicNotes && <div><small>GC said to team:  {call.publicNotes}</small></div>}
                                </Col>
                                <Col xs={4} md={4} style={{textAlign: "right"}}>
                                    <div><small>Call Started</small></div>
                                    <div><small>{moment.utc(call.callStart).fromNow()}</small></div>
                                </Col>
                            </Row>
                        </div>
        ...
        return (
            <div>There have been no calls for this team yet.</div>
        );
    }
}

export const CallHistory = ({currentTeam}) => {
    ...
    return (
        <div>
            <h4>
                Call History
                &nbsp;
                <Button onClick={() => setShowCallHistory(!showCallHistory)}>
