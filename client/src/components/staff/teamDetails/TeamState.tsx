    ...
    return (
        <>
            <LinkContainer to={`/staff/clues/${clue.tableOfContentId}`}>
                <Button active><FaPuzzlePiece className="mr-2"/> Info</Button>
            </LinkContainer>
            <DialogRenderProp
                    variant="primary"
                    renderTitle={() => "Override Sort Order"}
                    renderButton={() => <><FaSortNumericDown className="mr-2"/> Reorder</>}
            ...
            />
            { showRelock && <Button onClick={() => onRelock(clue.tableOfContentId)}><FaLock className="mr-2"/> Lock</Button> }
            { showUnlock && <Button onClick={() => onUnlock(clue.tableOfContentId)}><FaUnlockAlt className="mr-2"/> Unlock</Button> }
            { showSkip && <Button variant="warning" onClick={() => onSkip(clue.tableOfContentId)}><FaFastForward className="mr-2"/> Skip</Button> }
        </>
    );
};
...
const CluePlotContent = ({ status, contentList }: { status: SolveStatus, contentList: Content[] }) => {
    if (status.solveTime) {
        ...
        if (skippedPlot.length > 0) {
            return (
                <>
                    {skippedPlot.map(content => <AdditionalContent key={content.contentId} content={content}/>)}
                </>
            );
        } else {
            return (
...
export const TeamState = ({ teamId }: { teamId: string }) => {
    ...
    const currentCluesForTeam = cluesModule.clues;
                                    ...
                                    return (
                                        <ListGroupItem key={clue.tableOfContentId} variant={getVariantForClue(clue)}>
                                            <Container fluid>
                                                <Row>
                                                    <Col>
                                                        <h5 style={titleStyle}><ClueIcon className="mr-2" clue={clue}/>{clue.submittableTitle} {overrideDisplay}</h5>
                                                        ...
                                                        <ClueActions
                                                            ...
                                                            onChangeSortOrder={(tableOfContentId, sortOrder) => {
                                                                ...
                                                                if (currentTeam?.additionalData?.sortOverride?.length ?? -1 > 0) {
                                                                    ...
                                                                    if (!sortOverride.find(x => x.tableOfContentId === tableOfContentId) &&
                                                                        (clue.sortOrder !== sortOrder)) {
                                                                        sortOverride.push({ tableOfContentId, sortOrder});
                                                                    }    
                                                                } else if (sortOrder !== clue.sortOrder) {
                                                                    sortOverride.push({ tableOfContentId, sortOrder});
