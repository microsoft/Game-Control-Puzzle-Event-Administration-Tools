...
export const StaffGrid = () => {
    ...
    if (!gridModule.lastFetched) {
        ...
        const visibleClueIds = gridModule.data.clues.filter(
            clue => {
        ...
        return (
            <div style={{width: "100%", overflowX: "visible"}}>
                ...
                { // Hide the big clock during virtual events.
                    !isVirtual &&
                    <div>
                        Big Clock: {moment.utc(gridModule.data.latestEndTime).fromNow()}
                    </div>
                }
                        ...
                        <tr>
                            <th>
                                ...
                                <Form.Check
                                    ...
                                    type="switch"
                                    checked={hidePlot}
                                    onChange={(event: any) => setHidePlot(event.target.checked)}
                                    label="Hide Plot"
                                />
                                <Form.Check
                                    id="hideTestTeams"
                            ...
                            {gridModule.data.clues.filter(c => visibleClueIds.includes(c.submittableId)).map(puzzle =>
                                <PuzzleColumn
                                    key={puzzle.submittableId}
                                    puzzle={puzzle}
                                />
                            )}
                        </tr>

                        {gridModule.data.teams.filter(x => !x.isTestTeam || !hideTestTeams).map(team =>
                            <tr key={team.teamId}>
                                {console.log(team)}                                
