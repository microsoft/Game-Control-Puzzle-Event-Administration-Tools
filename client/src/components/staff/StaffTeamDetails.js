...
export const StaffTeamDetails = ( { match: { params: { id, tab }} }) => {
    ...
    if (currentTeam) {
        ...
        return <div>
            ...
            <div>Details for {currentTeam.name} ({currentTeam.shortName})</div>
            <div>Passphrase: {currentTeam.passphrase}</div>
            <div>Current {pointsSettingName}: {currentTeam.points}</div>
            <div style={{ padding: "15px "}}>
                ...
                <DialogRenderProp
                    ...
                    renderButton={() => <><FaGem/> Grant {pointsSettingName}</>}
                    renderBody={ onComplete =>
                        <GrantPointsForm
            ...
            <Tabs defaultActiveKey={1} id="team-detals-tabs" activeKey={key}  onSelect={handleTabChange}>
                <Tab eventKey={1} title="Calls">
                    { activeCall ? 
                        <Card>
                            <TeamCallForm key={activeCall.callId}
                                puzzles={clues.clues} 
                ...
                <Tab eventKey={2} title="Roster">
                    <TeamRoster
                        ...
                        updateUserInfo={(participantId, updates) =>
                            dispatch(updateUserInfo(participantId, updates))}
                    />
                </Tab>
                                    ...
                                    <Container>
                                        ...
                                        <Row>
                                            <Button
                                                ...
                                                onClick={() => dispatch(deletePlayerSubmission(submission.submissionId))}
                                                variant="danger">
                                                Delete
                                            </Button>
                                        </Row>
