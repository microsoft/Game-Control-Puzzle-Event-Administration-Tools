    ...
    if (!challengesModule.isLoading && challengesModule.data.length === 0) {
        ...
        const columns: ColumnDescription[] = [
            {
                ...
                formatter: (cell: any, row: any) => <><Link to={`/staff/challenges/${row.challengeId}`}>{cell}</Link><br/><small>{row.description}</small></>,
                sort: true
            },
            {
        ...
        return (
            <BootstrapTable
                ...
                defaultSorted={[{ dataField: "pendingSubmissions", order: "desc" }]}
...
export const Challenges = () => {
    ...
    return (
        <div>
            ...
            {
                !!challengesModule.lastError &&
                <Alert variant="danger">{challengesModule.lastError}</Alert>
            }

            {
                !!challengesModule.isLoading &&
