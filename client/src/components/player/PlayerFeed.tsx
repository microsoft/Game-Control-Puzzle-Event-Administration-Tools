        ...
        <div>
            <Row>
                <Col xs={8} md={8} style={{textAlign: "left"}}>
                    <div><strong>{feedItem.teamName}</strong> unlocked <strong>{feedItem.description}</strong></div>
                </Col>
                <Col xs={4} md={4} style={{textAlign: "right"}}>
                    <div><small>Achievement</small></div>
                    <div><small>{moment.utc(feedItem.lastUpdated).fromNow()}</small></div>
                </Col>
            </Row>
        </div>
    );
...
export const PointsFeedItem = ({ feedItem }: FeedItemProps) => {
    ...
    return (
        <div>
            <Row>
                <Col xs={8} md={8} style={{textAlign: "left"}}>
                    <div><FaCoins size="32px" className="mr-3"/><strong>{feedItem.teamName}</strong> earned <strong>{feedItem.numericValue}</strong> {pointsSettingName} for {feedItem.description}</div>
                </Col>
                <Col xs={4} md={4} style={{textAlign: "right"}}>
                    <div><small>{pointsSettingName}</small></div>
                    <div><small>{moment.utc(feedItem.lastUpdated).fromNow()}</small></div>
                </Col>
            </Row>
        </div>
    ...
    return (
        <div>
            <Row>
                ...
                <Col xs={4} md={4} style={{textAlign: "right"}}>
                    <div><small>Message from GC</small></div>
                    <div><small>{moment.utc(feedItem.lastUpdated).fromNow()}</small></div>
                </Col>
            </Row>
        </div>
    ...
    return (
        <div>
            <Row>
                <Col xs={8} md={8} style={{textAlign: "left"}}>
                    <div>{feedItem.description}</div>
                    <div><small>Pulsed by {feedItem.teamName}</small></div>
                    {
                        feedItem.hasAdditionalImage &&
                        <img alt="" src={constants.APPLICATION_URL + "api/content/pulseThumb/" + feedItem.id}/>
                    }
                </Col>
                <Col xs={4} md={4} style={{textAlign: "right"}}>
        ...
        <div>
            <Row>
                <Col xs={8} md={8} style={{textAlign: "left"}}>
                    ...
                    <div><small>Submitted by {feedItem.teamName}</small></div>
                </Col>
                <Col xs={4} md={4} style={{textAlign: "right"}}>
                    <div><small>Submission</small></div>
                    <div><small>{moment.utc(feedItem.lastUpdated).fromNow()}</small></div>
                </Col>
            </Row>
        </div>
            ...
            <Row>
                <Col xs={8} md={8} style={{textAlign: "left"}}>
                    <div style={{ verticalAlign: "center" }}>
                        <strong>
                            {`${feedItem.teamName} has unlocked ${feedItem.description}`}
                        </strong>
                    </div>
                </Col>
                <Col xs={4} md={4} style={{textAlign: "right"}}>
                    <div><small>Team Unlock</small></div>
                    <div><small>{moment.utc(feedItem.lastUpdated).fromNow()}</small></div>
                </Col>
            </Row>
        </div>
...
export const PlayerFeed = () => {    
    ...
    return (
        <div>
            ...
            {
                !!feedModule.lastError &&
                <div>There was an error loading the activity feed</div>
            }
            {
                !!feedModule.data &&
                <ListGroup>
