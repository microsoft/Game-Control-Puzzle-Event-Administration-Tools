...
const isChallengeExpired = (challenge: any) => 
    challenge.endTime === undefined || moment.utc().diff(moment.utc(challenge.endTime)) > 0;

const isChallengeCompleted = (challenge: any) =>
    challenge.submissions.find((x: any) => x.state === 1) !== undefined;

const isChallengeAwaitingReview = (challenge: any) =>
    challenge.submissions.find((x: any) => x.state === 0) !== undefined;
    ...
    return (
        <LinkContainer key={challenge.challengeId} to={`/player/challenges/${challenge.challengeId}`}>
            <ListGroupItem variant={color}>
                <>
                <div><h5>{challenge.title}{tag}</h5></div>
                <div>{challenge.description}</div></>
                <div>
                {!!challenge.endTime ?
                    (moment.utc().diff(moment.utc(challenge.endTime)) > 0) ?
                        null :
                        (<small>This {challengesSingularName} is available until {moment.utc(challenge.endTime).local().format('MM-DD HH:mm')}</small>) :
                    null
                }</div>
            </ListGroupItem>
        </LinkContainer>
...
export const Challenges = () => {
    ...
    const _renderChallengeModule = (challengeModule: Module<PlayerChallenge[]>) => {
        if (challengeModule.data.length > 0) {
            ...
            const active = challengeModule.data.filter((x: any) => !isChallengeExpired(x) && !isChallengeAwaitingReview(x) && !isChallengeCompleted(x)).sort((a: any, b: any) => a.title.localeCompare(b.title));
