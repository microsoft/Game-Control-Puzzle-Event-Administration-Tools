    ...
    if (content?.length > 0) {
        return (
            <Card className="contentCard">
                <Card.Body>
                    <ListGroup>
                        {content.sort((a,b) => moment.utc(b.lastUpdated).diff(moment.utc(a.lastUpdated))).map(content => 
    ...
    if (submissions.length > 0) {
        return (
            <Card>
                ...
                <Card.Body>
                    <ListGroup>
                        {submissions.map(submission => <Submission key={submission.submissionId} submission={submission} playerId={playerId} teamId={teamId}/>)}
                    </ListGroup>
                </Card.Body>
            </Card>
        );
...
const Submission = ({ submission, teamId, playerId }: { submission: PlayerSubmission, teamId: string, playerId: string }) => {
    ...
    return (
        <ListGroupItem key={submission.submissionId}
                variant={listGroupItemType}>
            { submission.isHidden ?
                <h5>You successfully solved this puzzle</h5> :
                <h5>{submission.submission}</h5>
            }
            <div><small>{submission.answerResponse}</small></div>
            <div><small>{moment.utc(submission.submissionTime).fromNow()}</small></div>
...
const UnlockedClues = ({ clues }: { clues?: UnlockedClue[] }) => {
    if (clues && clues.length > 0) {
        return (
            <div>
                <div><small><em>Unlocked Puzzles</em></small></div>
        ...
        return (
            <div>      
                <Alert variant="success">
                    {
                        firstCorrectSubmission.isHidden ?
                        <h4>You successfully solved this puzzle</h4> :
                        <h4>{ firstCorrectSubmission.submission }</h4>
                    }
...
export const PlayerClueDetails = ({ match: { params: { id }} }: RouteComponentProps<{ id: string }>) => {
    ...
    const ClueContent = () => {
        if (currentClue) {
            return (
                <div>
                    { 
                        !!cluesModule.lastAnswerError
                        &&
                        <Alert variant="danger">Failed to submit answer: {cluesModule.lastAnswerError.message}</Alert>
                    }
                    ...
                    <div><SubmissionHistory submissions={currentClue.submissions} playerId={playerId} teamId={teamId}/></div>
                </div>
            );
    ...
    return (
        <div>
            ...
            <CallManager />
            {
                !!cluesModule.lastError && 
                <Alert variant="danger">An error occured. Please contact Game Control. Error Message: {cluesModule.lastError.message}</Alert>
            }
            <ClueContent/>
        </div>
    );
