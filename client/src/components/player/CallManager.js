...
import { getAllPlayerPuzzles, getPointsNameSetting } from 'modules';
...
export const CallManager = () => {
    ...
    const unsolvedPuzzles = allPuzzles.filter(clue =>
        clue.submissionTime === null && clue.submittableType.trim() !== 'Plot');
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
                ...
                <Row style={{justifyContent: 'center', display: 'flex'}}>                                
                    <Card style={{ alignSelf: 'center', width: '175px', margin: "10px" }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
                { (!unsolvedPuzzles || unsolvedPuzzles.length === 0) &&
                    <Button variant="outline-info"
                            className="playerCallButton"
                            onClick={() => updateCall({ callType: "TeamFree" })}>
                        <div className="playerCallButtonIcon"><FaPuzzlePiece/></div>
                ...
                <Button variant="outline-warning"
                        className="playerCallButton"
                        onClick={() => updateCall({ callType: "TeamHelp" })}>
                    <div className="playerCallButtonIcon"><FaQuestionCircle/></div>
