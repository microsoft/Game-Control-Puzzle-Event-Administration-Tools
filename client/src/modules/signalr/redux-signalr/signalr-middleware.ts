...
const signal =
    ({ callbacks, onStart = () => {}, url, logLevel = signalR.LogLevel.Error, connectionOptions = {} }: any) =>
    (store: any) => {
        ...
        return (next: any) => (action: any) => {
            ...
            return typeof action === 'function' ? action(store.dispatch.bind(store), store.getState.bind(store), connection.invoke.bind(connection)) : next(action);
        };
    };
