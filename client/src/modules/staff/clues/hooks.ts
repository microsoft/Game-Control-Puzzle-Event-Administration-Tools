...
export const shouldRefreshClues = (cluesModule: any) =>
    !cluesModule.isLoading && (!cluesModule.lastFetched || moment.utc().diff(cluesModule.lastFetched, 'seconds') > 60); 

export const useStaffClues = () => {
    const cluesModule = useSelector(getStaffClues);
